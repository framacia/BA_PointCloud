...
namespace BAPointCloudRenderer.Loading
{
    ...
    class CloudLoader
    {
        ...
        private static void LoadHierarchy(PointCloudMetaData metaData, ref Node node)
        {
            ...
            {
                byte[] data = ReadFromFile(metaData.cloudPath + "hierarchy.bin", (long)node.hierarchyByteOffset,
                ...
                {
                    Debug.Log("Got incorrect amount of data from hierarchy.bin: " + data.Length + " != " +
                              node.hierarchyByteSize);
                }
            }
        }
        ...
        public static void CalculateRGBA(ref Color[] colors, ref int offset, byte[] data, int pointByteSize,
            int numPoints, bool alpha)
        {
            int size = alpha ? 4 : 3;
            ...
            for (int j = 0; j < numPoints; j++)
            {
                ...
                colors[j] = new Color32((byte)(r >> 8), (byte)(g >> 8), (byte)(b >> 8),
                    (byte)255); //<< 8: Move from [0, 65535] to [0, 255]
            }
        }
         ...
         */
        public static byte[] FindAndLoadFile(string dataRPath, PointCloudMetaData metaData, string id,
            string fileending)
        {
