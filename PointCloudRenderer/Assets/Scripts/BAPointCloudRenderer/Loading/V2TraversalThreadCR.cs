...
//         /// </summary>
//         public V2TraversalThreadCR(V2RendererCR mainThread, V2LoadingThreadCR loadingThread, List<Node> rootNodes,
//             double minNodeSize, uint pointBudget, uint nodesLoadedPerFrame, uint nodesGOsPerFrame, V2CacheCR cache)
//         {
...
//         public void SetNextCameraData(Vector3 cameraPosition, Vector3 camForward, Plane[] frustum, float screenHeight,
//             float fieldOfView)
//         {
...
//                     double angle = Math.Acos(camForward.x * camToNodeCenterDir.x + camForward.y * camToNodeCenterDir.y +
//                                              camForward.z * camToNodeCenterDir.z);
...
//                                 (screenHeight / 2.0) * child.BoundingBox.Radius() / (slope * distance);
//                             if (projectedSize > minNodeSize)
//                             {
//                                 Vector3 camToNodeCenterDir = (center - cameraPosition).normalized;
//                                 double angle = Math.Acos(camForward.x * camToNodeCenterDir.x +
//                                                          camForward.y * camToNodeCenterDir.y +
//                                                          camForward.z * camToNodeCenterDir.z);
//                                 double angleWeight = Math.Abs(angle) + 1.0; //+1, to prevent divsion by zero
//                                 double priority = projectedSize / angleWeight;
