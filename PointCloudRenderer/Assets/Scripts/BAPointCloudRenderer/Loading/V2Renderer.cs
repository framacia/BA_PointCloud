...
namespace BAPointCloudRenderer.Loading
{
    ...
    class V2Renderer : AbstractRenderer
    {
        ...
        private AbstractPointCloudSet pointCloudSet;
        ...
        public V2Renderer(AbstractPointCloudSet pointCloudSet, int minNodeSize, uint pointBudget,
            uint nodesLoadedPerFrame,
            uint nodesGOsperFrame, Camera camera, MeshConfiguration config, uint cacheSize)
        {
            this.pointCloudSet = pointCloudSet;
            rootNodes = new List<Node>();
            this.camera = camera;
            this.config = config;
            cache = new V2Cache(cacheSize);
            loadingThread = new V2LoadingThread(cache);
            loadingThread.Start(pointCloudSet, pointCloudSet.Loader);
            traversalThread = new V2TraversalThread(pointCloudSet.gameObject, this, loadingThread, rootNodes,
                minNodeSize,
                pointBudget, nodesLoadedPerFrame, nodesGOsperFrame, cache);
            traversalThread.Start(pointCloudSet);
        ...
        public void Update()
        {
            if (Traversed)
            {
                ...
                traversalThread.SetNextCameraData(camera.transform.position, camera.transform.forward,
                    GeometryUtility.CalculateFrustumPlanes(camera.projectionMatrix * camera.worldToCameraMatrix *
                                                           pointCloudSet.transform.localToWorldMatrix),
                ...
                while (toRender.Count != 0)
                {
                    ...
                    if (n.HasPointsToRender() &&
                        (n.MetaData.version == "2.0" || n.Parent == null || n.Parent.HasGameObjects()))
                    {
                        n.CreateGameObjects(config, pointCloudSet.transform);
                    }
                    //}
                }
