...
namespace BAPointCloudRenderer.CloudData
{
    ...
    public class Node : IEnumerable<Node>
    {
        ...
        public void CreateGameObjects(MeshConfiguration configuration, Transform parent)
        {
            ...
            {
                gameObjects.Add(configuration.CreateGameObject(
                    metaData.cloudName + "/" + "r" + name + " (" + verticesToStore.Length + ")", verticesToStore,
                    colorsToStore, boundingBox, parent, metaData.version, metaData.getAdditionalTranslation()));
            }
            else
            {
                ...
                while (amount > 0)
                {
                    ...
                    gameObjects.Add(configuration.CreateGameObject(
                        metaData.cloudName + "/" + "r" + name + "_" + index + " (" + vertices.Length + ")", vertices,
                        colors, boundingBox, parent, metaData.version, metaData.getAdditionalTranslation()));
        ...
        {
            GameObject box =
                UnityEngine.Object.Instantiate<GameObject>(Resources.Load<GameObject>("Prefabs/BoundingBoxPrefab"));
            int h = GetHashCode();
            box.GetComponent<MeshRenderer>().material.color = new Color(((float)h / int.MaxValue), (h % 10000) / 10000f,
                (h % 100) / 100f, 0.472f);
